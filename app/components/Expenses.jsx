...
const CATEGORIES = ['Rent', 'Electricity', 'Staff Salary', 'Transport', 'Packaging', 'Maintenance', 'Marketing', 'Other'];
...
export default function Expenses() {
  ...
  const filtered = expenses.filter(e => {
    const matchSearch = e.title.toLowerCase().includes(searchTerm.toLowerCase()) || e.notes?.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="card p-5">
          ...
          <p className="text-2xl font-bold text-red-600 mt-1">₹{totalAll.toLocaleString('en-IN')}</p>
        </div>
        <div className="card p-5">
          <p className="text-sm text-gray-500">Filtered Total</p>
          <p className="text-2xl font-bold text-gray-900 mt-1">₹{totalFiltered.toLocaleString('en-IN')}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search expenses..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="input pl-10" />
        </div>
        <select value={filterCategory} onChange={e => setFilterCategory(e.target.value)} className="input w-full sm:w-44">
          <option value="all">All Categories</option>
          {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
        </select>
        <input type="month" value={filterMonth} onChange={e => setFilterMonth(e.target.value)} className="input w-full sm:w-40" />
      </div>
          ...
          <div className="bg-white rounded-xl w-full max-w-md p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" required value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} className="input" placeholder="e.g., Monthly Rent" />
              </div>
              ...
              {filtered.map(expense => (
                <tr key={expense.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3">
                    ...
                    {expense.notes && <p className="text-xs text-gray-500 mt-0.5">{expense.notes}</p>}
                  </td>
                  <td className="px-4 py-3">
                    <span className={`badge ${categoryColors[expense.category] || categoryColors.Other}`}>{expense.category}</span>
                  </td>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-12 text-gray-500">
          {searchTerm || filterCategory !== 'all' || filterMonth ? 'No expenses match your filters' : 'No expenses recorded yet'}
        </div>
      )}
    </div>
  );

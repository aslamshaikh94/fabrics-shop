...
export default function SupplierLedger({ supplier, onClose }) {
  ...
  let balance = 0;
  const timelineWithBalance = timeline
    .map((item) => {
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-2 p-4 sm:p-6 border-b border-gray-100 shrink-0">
              ...
              <div className="text-center">
                ...
                <p
                  className={`text-lg font-bold mt-1 ${outstanding > 0 ? "text-warning-600" : "text-accent-600"}`}
                >
                  {fmt(outstanding)}
                </p>
              </div>
              ...
              <div className="px-4 sm:px-6 py-3 border-b border-gray-100 shrink-0">
                <div className="h-2 bg-gray-100 rounded-full overflow-hidden flex">
                  <div
                    ...
                    style={{ width: `${(totalPaid / totalPurchased) * 100}%` }}
                  />
                  <div
                    className="h-2 bg-warning-400"
                    style={{
                      width: `${(outstanding / totalPurchased) * 100}%`,
                    }}
                  />
                </div>
                <div className="flex justify-between text-xs text-gray-400 mt-1">
                  <span>
                    {((totalPaid / totalPurchased) * 100).toFixed(0)}% paid
                  </span>
                  <span>
                    {((outstanding / totalPurchased) * 100).toFixed(0)}% pending
                  </span>
                </div>
              </div>
            )}
                ...
                <div className="space-y-2">
                  ...
                  {timelineWithBalance.map((item) => (
                    <div
                      ...
                      className={`grid grid-cols-4 gap-2 items-center p-3 rounded-lg border ${item.type === "purchase" ? "border-warning-100 bg-warning-50" : "border-accent-100 bg-accent-50"}`}
                    >
                      <div className="col-span-2 flex items-center gap-3 min-w-0">
                        <div
                          className={`p-1.5 rounded-lg shrink-0 ${item.type === "purchase" ? "bg-warning-100" : "bg-accent-100"}`}
                        >
                          {item.type === "purchase" ? (
                      ...
                      <p
                        className={`text-right font-semibold text-sm ${item.type === "purchase" ? "text-warning-700" : "text-accent-700"}`}
                      >
                        {item.type === "purchase" ? "+" : "-"}
                        {fmt(item.amount)}
                      </p>

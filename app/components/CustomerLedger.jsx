...
export default function CustomerLedger({ customer, onClose }) {
  ...
  async function fetchLedger() {
    try {
      const [salesRes, paymentsRes] = await Promise.all([
        ...
        supabase.from('sale_payments').select('*, sale:sales(customer_id)').order('payment_date', { ascending: false }),
      ]);
      const allSales = salesRes.data || [];
      const saleIds = allSales.map(s => s.id);
      const relatedPayments = (paymentsRes.data || []).filter(p => saleIds.includes(p.sale_id));
      setSales(allSales);
  ...
  const timeline = [
    ...sales.map(s => ({ type: 'sale', date: s.sale_date, amount: s.total_amount, notes: s.notes, id: s.id })),
          ...
          <>
            ...
            <div className="grid grid-cols-3 gap-2 p-4 sm:p-6 border-b border-gray-100">
              <div className="text-center">
                ...
                <p className="text-lg sm:text-xl font-bold text-gray-900 mt-1">₹{totalBilled.toLocaleString('en-IN')}</p>
              </div>
              <div className="text-center">
            ...
            <div className="overflow-y-auto flex-1 p-4 sm:p-6">
              {timeline.length === 0 ? (
                ...
                <div className="space-y-3">
                  {timeline.map(item => (
                    <div key={`${item.type}-${item.id}`} className={`flex items-start gap-4 p-4 rounded-lg border ${item.type === 'sale' ? 'border-primary-100 bg-primary-50' : 'border-accent-100 bg-accent-50'}`}>
                      <div className={`p-2 rounded-lg ${item.type === 'sale' ? 'bg-primary-100' : 'bg-accent-100'}`}>
                        {item.type === 'sale'
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="font-medium text-gray-900 text-sm">
                          {item.type === 'sale' ? 'Sale' : `Payment — ${item.method?.toUpperCase()}`}
                        </p>
                      ...
                      <p className={`font-semibold text-sm whitespace-nowrap ${item.type === 'sale' ? 'text-primary-700' : 'text-accent-700'}`}>

...
export default function Shell() {
  ...
  const navSections = [
    { label: null, items: navItems.filter(n => ['dashboard', 'quicksale'].includes(n.id)) },
    { label: 'Transactions', items: navItems.filter(n => ['sales', 'purchases', 'payments'].includes(n.id)) },
    { label: 'Management', items: navItems.filter(n => ['fabrics', 'suppliers', 'customers', 'expenses'].includes(n.id)) },
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900">
      ...
      <div className="lg:hidden bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 px-4 py-3 flex items-center justify-between sticky top-0 z-30 backdrop-blur-sm bg-white/90 dark:bg-gray-800/90">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-sm font-semibold text-gray-900 dark:text-gray-100">{ALL_NAV.find(n => n.id === activePage)?.label || 'CRMS'}</h1>
        </div>
        <div className="w-9" />
      </div>

      {sidebarOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-40 lg:hidden transition-opacity" onClick={() => setSidebarOpen(false)} />
      )}

      {/* Sidebar */}
      <aside className={`fixed top-0 left-0 h-full w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 z-50 transform transition-transform duration-300 ease-out lg:translate-x-0 flex flex-col ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="p-5 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h1 className="text-base font-bold text-gray-900 dark:text-gray-100 leading-tight">CRMS</h1>
              <p className="text-[10px] text-gray-400 leading-tight">Fabric Shop Manager</p>
            </div>
          </div>
          <button onClick={() => setSidebarOpen(false)} className="lg:hidden p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-colors">
        ...
        <nav className="flex-1 px-3 overflow-y-auto scrollbar-thin">
          {navSections.map((section, si) => {
            ...
            return (
              <div key={si} className={si > 0 ? 'mt-4 pt-4 border-t border-gray-100 dark:border-gray-700/50' : ''}>
                {section.label && (
                  <p className="px-3 mb-1.5 text-[10px] font-semibold text-gray-400 uppercase tracking-widest">{section.label}</p>
                )}
                {filteredItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      <Icon className={`w-[18px] h-[18px] ${active ? 'text-primary-600 dark:text-primary-400' : 'text-gray-400 dark:text-gray-500 group-hover:text-gray-600 dark:group-hover:text-gray-300'}`} />

...
export default function Customers() {
  ...
  const [formData, setFormData] = useState({ name: '', phone: '', address: '', credit_limit: '', notes: '' });
  ...
  async function handleSubmit(e) {
    ...
    try {
      const payload = {
        ...
        credit_limit: parseFloat(formData.credit_limit) || 0, notes: formData.notes,
      };
      if (editingId) {
      ...
      setShowForm(false);
      setEditingId(null);
      setFormData({ name: '', phone: '', address: '', credit_limit: '', notes: '' });
      fetchCustomers();
    } catch (error) {
  ...
  function handleEdit(customer) {
    setFormData({ name: customer.name, phone: customer.phone, address: customer.address, credit_limit: customer.credit_limit.toString(), notes: customer.notes });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button onClick={() => { setShowForm(true); setEditingId(null); setFormData({ name: '', phone: '', address: '', credit_limit: '', notes: '' }); }} className="btn btn-primary">
      ...
      <div className="relative">
        ...
        <input type="text" placeholder="Search customers..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="input pl-10" />
      </div>
          ...
          <div className="bg-white rounded-xl w-full max-w-md p-4 sm:p-6 m-4 sm:my-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input type="text" required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="input" placeholder="Customer name" />
              </div>
              <div>
              ...
              <div>
                ...
                <input type="number" step="0.01" value={formData.credit_limit} onChange={(e) => setFormData({ ...formData, credit_limit: e.target.value })} className="input" placeholder="Maximum credit allowed" />
              </div>
              <div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button type="submit" className="btn btn-primary flex-1">{editingId ? 'Update' : 'Add'} Customer</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {ledgerCustomer && <CustomerLedger customer={ledgerCustomer} onClose={() => setLedgerCustomer(null)} />}

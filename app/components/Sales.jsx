...
export default function Sales() {
  ...
  async function fetchSales() {
    try {
      const { data, error } = await supabase.from('sales').select('*, customer:customers(*)').order('sale_date', { ascending: false });
  ...
  async function fetchPayments(saleId) {
    try {
      const { data } = await supabase.from('sale_payments').select('*').eq('sale_id', saleId).order('payment_date', { ascending: false });
      setPayments(data || []);
    } catch (error) {
  ...
  function handleAddPayment(sale) { setSelectedSale(sale); setShowPaymentForm(true); }
  ...
  const filteredSales = sales.filter(s => {
    ...
    const matchesSearch = customer.includes(searchTerm.toLowerCase()) || notes.includes(searchTerm.toLowerCase());
  ...
  const paymentBadge = (type) => {
    const styles = { cash: 'bg-accent-100 text-accent-800', credit: 'bg-warning-100 text-warning-800', partial: 'bg-blue-100 text-blue-800' };
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div className="relative flex-1">
          ...
          <input type="text" placeholder="Search by customer or fabric..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="input pl-10" />
        </div>
        <select value={filterType} onChange={(e) => setFilterType(e.target.value)} className="input w-full sm:w-40">
          <option value="all">All Types</option>
          <option value="cash">Cash</option>
          <option value="credit">Credit</option>
          <option value="partial">Partial</option>
        </select>
        <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)} className="input w-full sm:w-36" placeholder="From" />
        <input type="date" value={dateTo} onChange={e => setDateTo(e.target.value)} className="input w-full sm:w-36" placeholder="To" />
      </div>
          ...
          <div className="bg-white rounded-xl w-full max-w-lg p-6 my-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <select value={formData.customer_id} onChange={(e) => { const c = customers.find(c => c.id === e.target.value); setFormData({ ...formData, customer_id: e.target.value, customer_name: c?.name || '' }); }} className="input">
              ...
              <div className="border-t border-gray-200 pt-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-4">
                  <div>
                    ...
                    <input type="text" value={formData.fabric_color} onChange={(e) => setFormData({ ...formData, fabric_color: e.target.value })} className="input" placeholder="e.g., Blue" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Meters *</label>
                    <input type="number" step="0.01" required min="0.01" value={formData.meters} onChange={(e) => setFormData({ ...formData, meters: e.target.value })} className="input" placeholder="0" />
                  </div>
                  <div className="col-span-2 sm:col-span-1">
                    <label className="block text-sm font-medium text-gray-700 mb-1">Price/m *</label>
                    <input type="number" step="0.01" required value={formData.price_per_meter} onChange={(e) => setFormData({ ...formData, price_per_meter: e.target.value })} className="input" placeholder="₹0" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4 mt-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Cost Price/m</label>
                    <input type="number" step="0.01" value={formData.cost_price_per_meter} onChange={(e) => setFormData({ ...formData, cost_price_per_meter: e.target.value })} className="input" placeholder="₹0 (for margin calc)" />
                  </div>
                  <div>
              ...
              {formData.payment_type !== 'cash' && (
                <div>
                  ...
                  <input type="number" step="0.01" value={formData.initial_payment} onChange={(e) => setFormData({ ...formData, initial_payment: e.target.value })} className="input" placeholder="Amount received now" />
                </div>
              )}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Sale Date</label>
                <input type="date" value={formData.sale_date} onChange={(e) => setFormData({ ...formData, sale_date: e.target.value })} className="input" />
              </div>
              <div>
                ...
                <div className="bg-gray-50 rounded-lg p-3 space-y-1">
                  ...
                  {formData.cost_price_per_meter && (
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-semibold text-accent-600">₹{calculateMargin()}</span>
                    </div>
                  )}
                </div>
              )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl w-full max-w-md p-6">
            ...
            <div className="bg-gray-50 rounded-lg p-3 mb-4">
              <p className="text-sm text-gray-600">Customer: <span className="font-medium">{selectedSale.customer?.name || 'Walk-in'}</span></p>
              <p className="text-sm text-gray-600 mt-1">{selectedSale.notes}</p>
              <p className="text-sm text-gray-600 mt-2">Remaining: <span className="font-semibold text-warning-600">₹{selectedSale.remaining_amount.toLocaleString('en-IN')}</span></p>
            </div>
            <form onSubmit={handlePaymentSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Amount *</label>
                <input type="number" step="0.01" required max={selectedSale.remaining_amount} value={paymentData.amount} onChange={(e) => setPaymentData({ ...paymentData, amount: e.target.value })} className="input" placeholder="0.00" />
              </div>
              <div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                <button type="submit" className="btn btn-accent flex-1">Receive Payment</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl w-full max-w-lg p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => { setSelectedSale(null); setPayments([]); }} className="p-2 hover:bg-gray-100 rounded-lg"><X className="w-5 h-5" /></button>
            </div>
            <div className="bg-gray-50 rounded-lg p-3 mb-4">
              <p className="text-sm">Customer: <span className="font-medium">{selectedSale.customer?.name || 'Walk-in'}</span></p>
              <p className="text-sm text-gray-600 mt-1">{selectedSale.notes}</p>
              <div className="flex justify-between mt-2 pt-2 border-t border-gray-200">
                <span className="text-sm">Total: <span className="font-semibold">₹{selectedSale.total_amount.toLocaleString('en-IN')}</span></span>
                <span className="text-sm">Margin: <span className="font-semibold text-accent-600">₹{selectedSale.margin.toLocaleString('en-IN')}</span></span>
              </div>
              <p className="text-sm mt-2">Remaining: <span className="font-semibold text-warning-600">₹{selectedSale.remaining_amount.toLocaleString('en-IN')}</span></p>
            </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full" style={{ minWidth: '700px' }}>
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Customer / Fabric</th>
                <th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Date</th>
                <th className="text-right px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Meters</th>
                ...
                <th className="text-center px-4 py-3 text-xs font-medium text-gray-500 uppercase tracking-wider">Type</th>
              ...
              {paginated.map((sale) => (
                <tr key={sale.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-3">
                    <p className="font-medium text-gray-900">{sale.customer?.name || 'Walk-in'}</p>
                    {sale.notes && <p className="text-sm text-gray-500 max-w-xs truncate">{sale.notes}</p>}
                  </td>
                  ...
                  <td className="px-4 py-3 text-right text-gray-600 text-sm">{sale.meters}m</td>
                  <td className="px-4 py-3 text-right font-medium text-gray-900 text-sm">₹{sale.total_amount.toLocaleString('en-IN')}</td>
                  <td className="px-4 py-3 text-right text-sm"><span className="text-accent-600 font-medium">₹{sale.margin.toLocaleString('en-IN')}</span></td>
                  <td className="px-4 py-3 text-right text-sm">
                    <span className={sale.remaining_amount > 0 ? 'text-warning-600 font-semibold' : 'text-gray-500'}>
                      ₹{sale.remaining_amount.toLocaleString('en-IN')}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-center">{paymentBadge(sale.payment_type)}</td>
                  <td className="px-4 py-3">
                    <div className="flex items-center justify-center gap-1">
                      ...
                      <button onClick={() => setConfirmDelete(sale.id)} className="p-1.5 hover:bg-red-50 rounded-lg text-gray-500 hover:text-red-600" title="Delete sale"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-2">
          <p className="text-sm text-gray-500">{filteredSales.length} records — page {page} of {totalPages}</p>
      ...
      {filteredSales.length === 0 && <div className="text-center py-12 text-gray-500">No sales found</div>}
    </div>
  );
}

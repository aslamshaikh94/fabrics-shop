-- Add sale_group_id to sale_payments so a payment can be linked to a group
ALTER TABLE sale_payments ADD COLUMN IF NOT EXISTS sale_group_id uuid;

-- Update trigger: when a payment has sale_group_id, distribute proportionally to all items in the group
-- NOTE: preserves customer current_balance sync from migration 027.
